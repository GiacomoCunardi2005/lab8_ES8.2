...
int main()
{
    ...
    STR_p_Testa->i_ValoreInformativo = 60;                                 // Assegna il valore 60 all'elemento
    ...
    STR_p_CodaLista->i_ValoreInformativo = 50;                             // Assegna il valore 50 all'elemento
    STR_p_CodaLista->STR_p_PuntatoreProssimoElemento = nullptr;            // Questo è l'ultimo elemento, quindi il puntatore è nullptr
    STR_p_Testa->STR_p_PuntatoreProssimoElemento = STR_p_CodaLista;        // Collega il nuovo elemento (50) al primo elemento (60)
    ...
    STR_p_CodaLista->STR_p_PuntatoreProssimoElemento = STR_p_Testa;        // Collega il nuovo elemento al vecchio primo elemento (60)
    STR_p_Testa = STR_p_CodaLista;                                         // Aggiorna la testa della lista per puntare al nuovo elemento (70)

    // Stampa dell'intera lista (output atteso: "70 60 50")
    FX_void_Stampa(STR_p_Testa);

    // Eliminare il secondo elemento della lista (quello con valore 50)
    STR_p_CodaLista = STR_p_Testa->STR_p_PuntatoreProssimoElemento;         // Puntatore al secondo elemento (60)
    STR_p_Testa->STR_p_PuntatoreProssimoElemento = STR_p_CodaLista->STR_p_PuntatoreProssimoElemento;  // Collega il primo elemento (70) direttamente al terzo (50)
    delete STR_p_CodaLista;                                                // Libera la memoria allocata per il secondo elemento (60)

    // Stampa della lista aggiornata (output atteso: "70 50")
    FX_void_Stampa(STR_p_Testa);
